...
REGISTRY_REFRESH_SECS = int(os.environ.get("REGISTRY_REFRESH_SECS", "10"))  # short window — judges may test at any time, live demo must stay responsive
MAX_BACKOFF_SECS     = int(os.environ.get("MAX_BACKOFF_SECS", "120"))
...
def run_relay():
    ...
    registry_addresses_cache = {"addresses": None, "fetched_at": 0.0}
    ...
    while True:
        try:
            ...
            try:
                ...
                consecutive_rate_limit_errors = 0  # reset backoff on success
            except Exception as e:
                if is_rate_limit_error(e):
                    consecutive_rate_limit_errors += 1
                    backoff = min(POLL_INTERVAL * (2 ** consecutive_rate_limit_errors), MAX_BACKOFF_SECS)
                    log("ERROR", f"[eth_blockNumber failed] Rate limited (429). Backing off {backoff}s "
                                 f"(consecutive failures: {consecutive_rate_limit_errors}).")
                    time.sleep(backoff)
                else:
            ...
            # --- Step 2: Read factory (cached — only refresh every REGISTRY_REFRESH_SECS) ---
            now = time.monotonic()
            if registry_addresses_cache["addresses"] is None or \
               (now - registry_addresses_cache["fetched_at"]) > REGISTRY_REFRESH_SECS:
                try:
                    ...
                    for i in range(business_count):
                        try:
                            ...
                            if active and review_addr != "0x" + "0" * 40:
                                fresh_addresses.append(Web3.to_checksum_address(review_addr))
                        except Exception as e:
                            log("ERROR", f"[registries({i}) failed] {type(e).__name__}: {e}")
                    registry_addresses_cache["addresses"] = fresh_addresses
                    registry_addresses_cache["fetched_at"] = now
                    log("INFO", f"Refreshed registry list: {len(fresh_addresses)} active registry/ies")
                except Exception as e:
                    if is_rate_limit_error(e):
                        log("WARNING", f"[businessCount() failed] Rate limited (429). Using cached registry list.")
                    else:
                        log("ERROR", f"[businessCount() failed] {type(e).__name__}: {e}")
                    if registry_addresses_cache["addresses"] is None:
                        # No cache to fall back on yet — nothing to do this cycle.
                        time.sleep(POLL_INTERVAL)
                        continue
 
            registry_addresses = registry_addresses_cache["addresses"]
